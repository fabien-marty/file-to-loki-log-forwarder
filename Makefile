...
debug-docker: ## Build and run the docker image in pure debug mode
	...
	mkdir debug_logs
	$(MAKE) docker
	docker run --network host -it -p $(MANAGEMENT_API_PORT):$(MANAGEMENT_API_PORT) -v $$(pwd)/debug_logs:/logs --rm -it -e SINK_LOKI_ENDPOINT=http://localhost:3100 -e CODEC=text -e INCLUDE_VECTOR_LOGS=1 -e INCLUDE_INTERNAL_LOGS=1 -e STLOG_LEVEL=DEBUG -e IGNORE_NON_JSON_LINES=0 -e DEBUG=0 -e SINK_LOKI_LABELS=job=test,instance=test -e SOURCE_FILE_INCLUDE_PATHS=/logs/*.log $(IMAGE)
	rm -Rf debug_logs
